...
### P4. 뷰어 전용
Export된 HTML은 **뷰어 전용**이다. `editor.js`는 포함하지 않는다.
...
### C. `_assetCache` 프리페치
`editor.js`는 페이지 로드 시 동기적으로 CSS를 캐시하고 비동기로 JS를 캐시한다.
**이 프리페치를 망가뜨리는 변경 금지**:
- `<script src>` 순서를 바꾸지 말 것 (editor.js는 **마지막**이어야 `scripts[last]` 제거가 정확).
...
```
target/
├── {htmlName}.html          ← 뷰어 전용, editor.js 없음, 폰트 임베드
